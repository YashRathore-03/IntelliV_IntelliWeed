...
AF_DCMotor baseMotor(2);     // Base rotation motor (M2)
AF_DCMotor armMotor1(3);     // Arm Motor 1 (M3)
AF_DCMotor armMotor2(4);     // Arm Motor 2 (M4)

void setup() {
  // Set the motor speed for the gripper, base, and both arm motors
  gripperMotor.setSpeed(150);  // Speed for Gripper
  baseMotor.setSpeed(150);     // Speed for Base rotation
...
// Function to perform the entire arm and gripper movement with base rotation
void performArmMovement() {
  ...
  // Step 5: Rotate the base slightly
  rotateBase();
  delay(200);  // Base rotates slightly for 200ms

  // Step 6: Open the gripper at the top to drop the object
  openGripper();
  delay(1000);  // Gripper remains open to drop the object

  // Step 7: Close the gripper after dropping the object
  closeGripper();
  delay(500);  // Ensure the gripper closes
}

// Function to move both Arm Motor 1 and Arm Motor 2 forward and down
...
// Function to rotate the base slightly
void rotateBase() {
  baseMotor.run(FORWARD);  // Rotate the base slightly
  delay(200);              // Reduced rotation for 200ms for a slight movement
  baseMotor.run(RELEASE);  // Stop the base motor
}
...
// Function to stop the base motor
void stopBase() {
  baseMotor.run(RELEASE);  // Stop the base motor
}
